-- Keep category-specific browse filters aligned with fields sellers can enter.
-- These are normalized copies used for searching; the original display text is
-- retained for backwards compatibility and listing detail presentation.
...
ALTER TABLE public.classified_listing_details
  ...
  ADD CONSTRAINT classified_listing_details_job_category_length
    CHECK (job_category IS NULL OR char_length(btrim(job_category)) BETWEEN 2 AND 80);

CREATE INDEX IF NOT EXISTS classified_listing_details_home_property_type_idx
  ON public.classified_listing_details (home_property_type);
CREATE INDEX IF NOT EXISTS classified_listing_details_home_bedrooms_idx
  ON public.classified_listing_details (home_bedrooms);
CREATE INDEX IF NOT EXISTS classified_listing_details_home_bathrooms_idx
  ON public.classified_listing_details (home_bathrooms);
CREATE INDEX IF NOT EXISTS classified_listing_details_home_square_feet_idx
  ON public.classified_listing_details (home_square_feet);
...
CREATE INDEX IF NOT EXISTS classified_listing_details_job_pay_type_idx
  ON public.classified_listing_details (job_pay_type);
CREATE INDEX IF NOT EXISTS classified_listing_details_job_pay_min_idx
  ON public.classified_listing_details (job_pay_min);
CREATE INDEX IF NOT EXISTS classified_listing_details_job_pay_max_idx
  ON public.classified_listing_details (job_pay_max);
CREATE INDEX IF NOT EXISTS classified_listing_details_service_area_idx
  ON public.classified_listing_details (service_area);

-- Backfill acreage values that were previously stored as display text.
