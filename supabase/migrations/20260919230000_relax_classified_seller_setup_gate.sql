-- Classified listings (General/Vehicles/Homes/Jobs/Services) are
-- direct-contact, KSL-style: buyer and seller arrange payment, pickup, and
-- shipping themselves. Gem State never touches any of that for them. The
-- Stripe/payout requirement was already correctly dropped from the seller
-- setup UI for this reason, but the *shipping* requirement (default
-- shipping method, default handling days, plus a full ship-from return
-- address to even save the profile) was never relaxed to match -- so every
-- classified seller, including someone just posting a Home or Job listing,
-- was still forced through the full legacy checkout-marketplace onboarding
-- before create_classified_listing would let them past its own gate.
--
-- ship_by_days on asks is NOT NULL and isn't shown anywhere in the
-- classifieds UI, so a seller with no configured handling-time default
-- falls back to a generic value rather than blocking the insert.
...
BEGIN
  ...
  IF seller.user_id IS NULL OR seller.status <> 'active' OR seller.terms_accepted_at IS NULL THEN
  ...
  INSERT INTO public.asks (
    ...
    coalesce(seller.default_handling_days, 7)
