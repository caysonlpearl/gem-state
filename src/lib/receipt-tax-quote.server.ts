/* eslint-disable @typescript-eslint/no-explicit-any -- receipt tax queries span migration-backed tables */
...
export async function prepareReceiptTaxQuote(
  userId: string,
  input: {
    orderId: string;
  ...
  const { data: order, error } = await admin
    ...
    .eq("id", input.orderId)
    .single();
  if (error || !order || order.origin !== "sourcing_shopper")
    throw new Error("Shopping job not found.");
  ...
  if (!["sourcing", "payment_captured"].includes(order.status))
    throw new Error("This job has already moved on.");
  ...
  const { error: quoteError } = await admin.from("sourcing_receipt_tax_quotes").upsert(
    {
      ...
      original_tax_cents: order.tax_cents,
