...
export async function calculateReceiptTax(
  stripe: Stripe,
  input: {
    orderId: string;
    checkoutId: string;
  ...
  if (
    ![input.originalTax, input.merchandise, input.fee, input.shipping].every(
  ...
  return {
    calculationId: `credit_preview:${plan.invoice.id}:${note.id}`,
    taxCents: input.originalTax - taxRefund,
  };
}

export async function refundReceiptReduction(
  stripe: Stripe,
  input: {
    orderId: string;
  ...
  if (input.checkoutId) {
    ...
    if (session.invoice || session.invoice_creation?.enabled) {
      return refundReceiptCredit(stripe, {
        ...
        finalTaxCents: input.finalTaxCents!,
      });
    }
    if (Number(session.total_details?.amount_tax ?? 0) > 0)
      throw new Error(
        "This older taxed order needs an administrator to reconcile its receipt refund.",
      );
  }
  ...
  const existing = prior.data.find(
    (refund) =>
